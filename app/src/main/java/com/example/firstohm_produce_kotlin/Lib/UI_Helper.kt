...
class UI_Helper(view: View){
    ...
    public fun login(jsonStr: JSONObject, view: View, context: Context){
        try {
            ...
            view.more_option_btn2.visibility=View.VISIBLE
            view.info_textView.visibility=View.VISIBLE
            ...
            /////////////
            view.btn_material_issue.visibility=View.VISIBLE
            view.terrible_layout.invalidate()
            if (MainActivity.dept=="花蓮底漆"){
                view.part_finsh_btn.visibility=View.GONE
            }else{
                view.part_finsh_btn.visibility=View.GONE
            }
            if (MainActivity.dept=="花蓮底漆" || MainActivity.dept=="花蓮色碼"){
            ...
            if(MainActivity.userBar.indexOf("Z")>-1){
                view.flow_start_btn.setText("Bắt đầu sản xuất")
                view.sance_btn.setText("Quét đơn mới")
                view.flow_finsh__btn.setText("Kết thúc sản xuất")
                view.savebtn.setText("Đăng xuất")
                view.btn_acc.setText(" Không dư thừa vật liệu")
                view.textView122.setText("Đã hoàn thành")
                view.textView123.setText("Số lượng -xấu")
    ...
    fun getDigitsNumber(d: Double): Int {
        return Math.log10(d).toInt() + 1
    }
    fun get_start_cmd(start_text: String): String {
        ...
        if (MainActivity.dept == "花蓮貼帶") {
            ...
            cmd_map["延續前班製程 Tiếp tục ca trước"]="33"
            cmd_map["更換機台作業 thay đơi máy"]="34"
        }else{
            ...
            cmd_map["延續前班製程 Tiếp tục ca trước"]="3"
            cmd_map["更換機台作業thay đơi máy"]="4"
            cmd_map["更換機台作業 thay đơi máy"]="4"
            cmd_map["更換機台"]="4"
        }
        cmd_map["換機開始"]="4"
        cmd_map["全檢1"]="15"
    ...
    fun send_mail(body: String): String{
        ...
        try {
            ...
            hashMap.put("messagebody","$body<br> url：" +sendurl
                    +"<br> version："+MainActivity.version
                    +"<br> version："+android.os.Build.PRODUCT
                    +"<br> tablet_name："+MainActivity.table_custom_name
                    +"<br> flowbar："+MainActivity.flowbar
                    +"<br> flow_message："+MainActivity.flow_message
