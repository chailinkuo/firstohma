...
import co.ubunifu.kotlinhttpsample.Lib.WebapiClient_test
...
class custom_layout_quant_input(
    ...
    init {
        ...
        get_machins.setOnClickListener {
            ...
            var url1 = MainActivity.ip + "PrdMgn/ScanOperate?command=d&UID=" +
                    MainActivity.userBar + "&MID=" + MainActivity.machineBar +
                    "&DEPT=" + MainActivity.dept
            ///////////////////////////////////////////
            var WebapiClient_test = WebapiClient_test(context)
            val updater = WebapiClient_test(context)
            val urlA = URL(url1)
            WebapiClient_test.execute(urlA)
            ///////////////////////
            val inflater = LayoutInflater.from(context)
            val v: View = inflater.inflate(R.layout.custom_layout_quant_input, null)
            try {
                val v2: View = inflater.inflate(R.layout.custom_layout_quant_input, null)
                val ui_Helper = co.ubunifu.kotlinhttpsample.Lib.UI_Helper(rootView)
                var cmd=8//正常取資訊
                if (MainActivity.dept=="花蓮貼帶")cmd=38//貼帶取資訊
                if (MainActivity.dept=="花蓮外檢")cmd=48//外檢取資訊
                var url = MainActivity.ip + "PrdMgn/ScanOperate?command=$cmd&UID=" +
                        MainActivity.userBar + "&MID=" + MainActivity.machineBar +
                        "&DEPT=" + MainActivity.dept
                MainActivity.r_url=url
